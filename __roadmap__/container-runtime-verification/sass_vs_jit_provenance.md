...
- ⬜ `[run]` The 4 JIT entries on the PME path are attributed to either `libgromacs` PTX or an independently-shipped CUDA library. Cheapest discriminator: re-run md with `-pme cpu` against a fresh cache — if the entries vanish they belong to the PME/cuFFT path, if they persist they are `libgromacs`
