...
Lazy imports in `__init__.py` (v8's eager `import xtc_to_clips` broke pxr-only interpreters — leave that behind, R00). No USD imports in the config module so it is importable before the OpenUSD env loads.
**Deliverables**: `examples/p53_mdm2/__init__.py`, `examples/p53_mdm2/p53_env.py`
**Consistency Checks**: `python3 -c "import sys; sys.path.insert(0, 'examples'); import p53_mdm2.p53_env as e; assert callable(e.get_data_dir) and callable(e.output_dir)"` (expected: PASS)
...
**Implementation Logic**:
Ported verbatim from v8 with no ABL-specific edits — R00 classifies these modules reuse-as-is. `data/` exposes pure-Python constant tables (no `pxr`/`mdtraj` imports) so it stays importable anywhere, including inside read-back tests as the anti-tautology source of truth; the element-template builder builds the `/_class_/<symbol>` taxonomy prims from that data.
**Deliverables**: `examples/p53_mdm2/data/**`, element-template builder
**Consistency Checks**: `. ./load_env.sh && /Users/hacker/Documents/src/AOUSD/forOUSD/bin/python3 -c "import sys; sys.path.insert(0, 'examples'); from p53_mdm2.data import ELEMENTS, RESIDUES; from p53_mdm2.builders import element_templates; assert ELEMENTS and RESIDUES and hasattr(element_templates, 'build_element_classes')"` (expected: PASS)
