...
- ✅ `[run]` `cuobjdump -lptx` output in the committed evidence names **both** `sm_70` and `sm_90` as PTX targets — 49 records each, `.ptx` extension, no cubin or `compute_*` token in the listing — recorded as a benign CMake behaviour, not reported as a defect. **Reworded 2026-08-12; the original prediction is recorded as falsified.** The original gate demanded `sm_90` only, matching `GMX_CUDA_TARGET_COMPUTE="90"`. Observation contradicts it: GROMACS' CMake emits PTX for every `GMX_CUDA_TARGET_SM` entry regardless of `GMX_CUDA_TARGET_COMPUTE`, which is the correction already carried in both recipes since 2026-07-30. Functionally harmless — more PTX means more forward-portability at the cost of a marginally larger binary — and the build flag is unchanged. R14 proposed an alternative reading, that a `-lptx` record's `sm_` token names its *enclosing cubin* rather than its own target; that reading is **also falsified**, since the `-lptx` listing contains no `.cubin` name at all `[source: evidence/ptx_disambiguation_banyan.txt; manifest seq 6]`
