...
**Success Gates**:
- ✅ `[run]` The rebuilt image's SASS summary and version block are identical to the pre-correction capture, confirming the recipe corrections altered no compiled content — `EQUIVALENCE_SUMMARY=identical`, `EQUIVALENCE_GMX_VERSION_BLOCK=identical` `[source: evidence/convert_verify_banyan.txt:54-56]`
- ⬜ `[run]` A **cold-cache** rebuild (`docker build --no-cache`) reproduces the same SASS summary and version block — the campaign's first genuine build-reproducibility datum. **Split out of gate 1 on 2026-08-12, and NOT met.** The gate above was over-claiming: its observable half holds, but the trailing clause "giving the campaign its first build-reproducibility datum" is precisely what the evidence disclaims. Job 33 logged `#7 CACHED / #8 CACHED`, so equivalence was measured against a cached compile layer `[source: manifest.jsonl seq 3 `caveat`]`. Note this needs no prune: `--no-cache` ignores rather than deletes the daemon's 16.46 GB build cache, so it is independent of the cleanup gate below. Its cost is disk — another ~10.6 GB image plus cache on a filesystem at 53%, and a fresh dangling image
...
- ⬜ `[run]` No archive and no `gromacs-p53mdm2` image remain, `/` free space is recorded before and after, **and the reclaimed space is actually released**. **Reworded 2026-08-12 to match the parent, and NOT met.** The leaf's original wording was weaker than the parent's and *was* satisfied — `gromacs.tar` deleted, `image rows: 0`, before/after `df` and `docker system df` captured — so a reader of the leaf alone would have ticked it and concluded cleanup was complete, while the parent recorded the opposite. That asymmetry was the defect. What is **not** achieved: `/` held at 430 G; deleting the image moved ~10 GB from active to *reclaimable* (169.7 → 179.9 GB), and releasing it needs `docker image prune` across a store holding 52 other users' dangling images — an admin/owner action, out of scope for this project. See the parent's matching gate in [`../README.md`](../README.md) `[source: evidence/convert_verify_banyan.txt:133-163]`
...
**Deliverables**: `examples/p53_mdm2/cluster/evidence/convert_verify_banyan.txt`, `STEP 1` sections — the rebuilt image's SASS summary and version block alongside a diff against the original captures (`EQUIVALENCE_SUMMARY`, `EQUIVALENCE_GMX_VERSION_BLOCK`); plus an appended `manifest.jsonl` entry. *(Filename corrected 2026-08-12: planned as `rebuild_equivalence.txt`, delivered consolidated into `convert_verify_banyan.txt` during the attended session.)*
...
**Deliverables**: `/home/eliott/p53mdm2/gromacs.sif` on the shared home; `examples/p53_mdm2/cluster/evidence/convert_verify_banyan.txt`, `STEP 2` sections — archive and image digests, full inspect output, environment block. *(Filename corrected 2026-08-12: planned as `sif_build.txt`, delivered consolidated into `convert_verify_banyan.txt`.)*
...
**Deliverables**: `examples/p53_mdm2/cluster/convert_verify.sh` — single-GPU request, Singularity execution with GPU passthrough, bind mounts for the smoke inputs and outputs; `examples/p53_mdm2/cluster/evidence/convert_verify_banyan.txt`, `STEP 3` sections. *(Filenames corrected 2026-08-12: planned as a separate `sif_gpu_smoke.sbatch` plus `sif_gpu_smoke.txt` and `sif_md.log.txt`; delivered as one script and one consolidated evidence file covering steps 1–3 and 5.)*
...
**Deliverables**: `examples/p53_mdm2/cluster/evidence/convert_verify_banyan.txt`, `STEP 5 CLEANUP` section — free space and daemon disk usage before and after, with the removal commands and their exit statuses. *(Filename corrected 2026-08-12: planned as `cleanup.txt`, delivered consolidated into `convert_verify_banyan.txt`.)*
