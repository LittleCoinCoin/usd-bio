...
- ✅ `[run]` `mdrun` exit status 0 recorded in the committed evidence. **Reworded 2026-08-12**: the original said "in the manifest". `mdrun`'s own exit 0 lives in the body file's sentinel, and `manifest.jsonl` seq 2 records the job-level `exit_status: 0`. The manifest was deliberately **not** edited after the fact to satisfy this gate — it is a capture record whose purpose is to let evidence be distinguished from an edited copy
...
**Deliverables**: `examples/p53_mdm2/cluster/docker_gpu_smoke.sh` — GRES GPU request, `DEV` resolution chain from `SLURM_JOB_GPUS` through `SLURM_STEP_GPUS` to `CUDA_VISIBLE_DEVICES`, explicit device passthrough, and a header stating the no-isolation caveat. *(Filename corrected 2026-08-12: delivered as `.sh` submitted through `submit_job`, not as a `.sbatch` file.)*
...
**Deliverables**: `examples/p53_mdm2/cluster/evidence/docker_gpu_smoke_banyan.txt` — verbatim version block, grompp output and both run logs; plus appended `manifest.jsonl` entries carrying job id. *(Filenames corrected 2026-08-12: the separate `docker_md.log.txt` was consolidated into `docker_gpu_smoke_banyan.txt` during the attended session.)*
